...
// Le sezioni della scheda colturale (con, pt, pg, pgr) vengono popolate solo
// se l'utente ha compilato i campi corrispondenti nel form avanzato.
// Se manca un campo, la sua sezione viene saltata e l'utente vedrà la scheda
// con solo le info compilate, senza tab vuoti che confonderebbero.
function buildCustomSPlant(row) {
  ...
  const record = {
    ...
    icon: row.icon || '🌱',
    isNew: false,
    tags: ['Personalizzata'],   // Tag visibile per distinguere le custom
    tc: tcKeys,
  ...
  // form base salva fert_months e fert_interval con default ragionevoli.
  if (row.fertilization || row.fert_product) {
    record.con = {
      periodo: _monthsToPeriod(row.fert_months),
      frequenza: `Ogni ${row.fert_interval} giorni`,
      concime: row.fert_product || 'Concime adatto al gruppo ' + row.sim_group,
      stop: 'Stop in periodo di riposo vegetativo',
      note: row.fertilization || row.fert_note || '',
    };
  }

  // Sezione potatura. Solo se compilata.
  if (row.pruning) {
    record.pt = { note: row.pruning };
    tcKeys.push('pt');
  }

  // Sezione parassiti. Solo se compilata.
  if (row.pests) {
    record.pg = { note: row.pests };
    tcKeys.push('pg');
  }
...
function cpRenderPanel() {
  ...
  if (!items.length) {
    // Stato vuoto: invito discreto. Non vogliamo che l'utente che non userà
    // mai questa funzione veda un pannello vistoso. Solo un link sottile.
    panel.innerHTML = `
      <div style="display:flex;align-items:center;justify-content:center;gap:10px;padding:10px;border:1px dashed #ccc;border-radius:8px;background:#fafaf6;font-size:13px;color:var(--muted)">
        <span>🌱 Hai 26 piante predefinite. Vuoi aggiungerne una?</span>
        <button class="ctrl-btn" style="padding:4px 12px;font-size:12px" onclick="cpOpenAddForm()">➕ Aggiungi pianta</button>
      </div>`;
    return;
  }
...
function cpPopulateForm(plant) {
  ...
  // Livello avanzato — testi
  document.getElementById('cp-description').value = plant.description || '';
  document.getElementById('cp-exposure').value = plant.exposure || '';
  document.getElementById('cp-watering').value = plant.watering || '';
  document.getElementById('cp-substrate-text').value = plant.substrate || '';
  document.getElementById('cp-temperature').value = plant.temperature || '';
  document.getElementById('cp-pruning').value = plant.pruning || '';
  document.getElementById('cp-pests').value = plant.pests || '';
  document.getElementById('cp-fertilization').value = plant.fertilization || '';
  // Livello avanzato — calendario
  ...
  // Stessa logica per i contenuti avanzati
  const hasAdvancedData = plant.description || plant.exposure || plant.watering ||
    plant.substrate || plant.temperature || plant.pruning || plant.pests || plant.fertilization;
  if (hasAdvancedData) {
...
async function cpSavePlant() {
  ...
  const payload = {
    ...
    exposure: document.getElementById('cp-exposure').value.trim(),
    watering: document.getElementById('cp-watering').value.trim(),
    substrate: document.getElementById('cp-substrate-text').value.trim(),
    temperature: document.getElementById('cp-temperature').value.trim(),
